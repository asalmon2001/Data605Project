...
Compare vegetables (price and yield)

**Comparisons:**

Cheapest and most expensive vegetable

Avg. price/yield of fresh vs canned vs frozen
...
```{r data_import}
  data = read.csv('Vegetable Prices 2020.csv')
  head(data)
```
...
```{r order_by_cup_price}
  ...
  head(select(data.cup_price.ascending, Vegetable, Form, CupEquivalentPrice), 5)
  head(select(data.cup_price.descending, Vegetable, Form, CupEquivalentPrice), 5)
```

```{r order_by_yield}
  data.yield.ascending = data[order(data$Yield),]
  
  head(select(data.yield.ascending, Vegetable, Form, Yield), 5)
```
